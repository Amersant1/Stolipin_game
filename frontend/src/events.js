...
let list_of_non_used_dates = []
for(let i=0;i<list_of_dates.length;i++){
  list_of_non_used_dates.push(i);
}


let list_of_indexes = []//это список из рандомных индексов всех дат, чтобы ивенты каждый раз были перемещаны(надо запомнить в куки)
for(let i=0;i<list_of_dates.length;i++){
  let random_int = get_random(list_of_non_used_dates);
  list_of_indexes.push(list_of_non_used_dates[random_int]);
  list_of_non_used_dates.pop(random_int);
...
for( let i=0;i<events.length;i++){
    let event = events[i];
    if (!("date" in event)){
      event["date"] = list_of_dates[list_of_indexes[i]];
    }    
